import { GiEarthAmerica } from "react-icons/gi";
import { MdAirplanemodeActive, MdTimer } from "react-icons/md";
import { FaMoneyCheck } from "react-icons/fa";
export const stats = [
  {
    icon: <GiEarthAmerica style={{ color: "#047bf1" }} />,
    title: "Over 100 Destinations",
    desc: "Travel to over 100 unique places",
  },
  {
    icon: <MdAirplanemodeActive style={{ color: "#f3a82e" }} />,
    title: "1 Million trips Made",
    desc: "Over 1 Million trips successfully completed last year",
  },
  {
    icon: <MdTimer style={{ color: "#f34f2e" }} />,
    title: "Fastest Help and Support",
    desc: "Contact our support team 24/7 ",
  },
  {
    icon: <FaMoneyCheck style={{ color: "#3af576" }} />,
    title: "Best Deals Available",
    desc: "We offer the best prices across the World",
  },
];
